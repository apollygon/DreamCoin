...
Project name: drem-master or drem-dev

Description: Build DREM for Win, Mac, Linux
...
In the build step paste script:
    ...
    wget https://raw.githubusercontent.com/DreamPay/DreamPay/master/contrib/CI/jenkins.sh -O $WORKSPACE/jenkins.sh
    chmod +x $WORKSPACE/jenkins.sh
    export RELEASEDIR=/var/www/drem
...
Create directory `/var/www/drem` and make Jenkins owner of that directory

    sudo mkdir /var/www/drem
    sudo chown jenkins /var/www/drem
