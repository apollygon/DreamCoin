...
copy_build_out()
{
    if [[ 0 -lt $(ls build/out/drem-* 2>/dev/null | wc -w) ]]
    then
...
then
    ...
    git clone https://github.com/DreamPay/DreamPay.git 
else
...
rm -rf DreamPay/depends/work
rm -rf `pwd`/cache/common
make -C DreamPay/depends download SOURCES_PATH=`pwd`/cache/common
...
echo ""
bin/gbuild --commit DreamPay=$commit  DreamPay/contrib/gitian-descriptors/gitian-osx.yml
copy_build_out "osx"
print_log
...
echo ""
bin/gbuild --commit DreamPay=$commit  DreamPay/contrib/gitian-descriptors/gitian-linux.yml
...
echo ""
bin/gbuild --commit DreamPay=$commit  DreamPay/contrib/gitian-descriptors/gitian-win.yml
copy_build_out "win"
print_log
